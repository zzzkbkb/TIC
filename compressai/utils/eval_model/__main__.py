...
def inference(model, x):
    ...
    kwargs = {}
    if isinstance(out_enc, dict):
        if "y_shape" in out_enc:
            kwargs["y_shape"] = out_enc["y_shape"]
        if "pool_stride" in out_enc:
            kwargs["pool_stride"] = out_enc["pool_stride"]
        if "pool_kernel" in out_enc:
            kwargs["pool_kernel"] = out_enc["pool_kernel"]

    start = time.time()
    try:
        out_dec = model.decompress(out_enc["strings"], out_enc["shape"], **kwargs)
    except TypeError:
        # backward-compatible fallback for models that don't accept extra args
        out_dec = model.decompress(out_enc["strings"], out_enc["shape"])
    dec_time = time.time() - start
...
def eval_model_multi_rate(
    ...
    for level in rate_levels:
        # 🔑 设置 rate level
        if hasattr(model, "set_rate_level"):
            model.set_rate_level(level)
        else:
            raise RuntimeError("Model does not support rate level control")
        ...
        results[f"rate_{level}"] = dict(metrics)
...
def main(argv):
    ...
    for run in runs:
        ...
        rate_levels = [0, 1, 2, 3, 4]

        metrics = eval_model_multi_rate(
            model,
            filepaths,
